...
@Component({
  selector: 'app-betgame-profitandloss',
  templateUrl: './betgame-profitandloss.component.html',
  styleUrls: ['./betgame-profitandloss.component.scss']
})
export class BetgameProfitandlossComponent implements OnInit {
  ...
  GetProfitLoss() {
    ...
    this.reportsService.CasinoBGProfitLoss(
      this.datePipe.transform(this.fromDate, 'yyyy-MM-dd HH:mm:ss'),
      this.datePipe.transform(this.toDate, 'yyyy-MM-dd HH:mm:ss'),219921
    ).subscribe(
      (resp: any) => {
        if (this.stype == '') {
